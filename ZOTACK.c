...
int (*builtin_funcs[])(char **) = {&tux_cd, &tux_help, &tux_exit, &tux_history, &tux_grep, &args_length};
...
char *get_hist_file_path()
{
        ...
        strcat(strncpy(file_path, getenv("HOME"), 113), "/.tux_history");
        return file_path;
}
...
int tux_history()
{
        ...
        char **args = NULL;
        if (!fp)
                fprintf(stderr, RED "tux: file not found" RESET "\n");
        else
        {
                putchar('\n');
                while ((c = getc(fp)) != EOF)
                ...
                while ((fgets(line, 128, fp)) != NULL)
                {
                        if (line_num == ch)
                        {
                                ...
                                if (!flag)
                                {
                                        ...
                                        return tux_launch(args);
                                }
                                else
                                {
                                        args = split_pipes(prev_comm);
                                        return tux_pipe(args);
                                }
                        }
                        else
                                line_num++;
...
int tux_execute(char **args)
{
        ...
        if (cpid == 0)
        {
                if (execvp(args[0], args) < 0)
                        printf("tux: command not found: %s\n", args[0]);
                exit(EXIT_FAILURE);
        }
        else if (cpid < 0)
...
int tux_launch(char **args)
{
        ...
        return tux_execute(args);
}

int tux_grep(char **args)
{
        FILE *fp = NULL;
        int flag = 0;
        char temp[512];
        ...
        if (args[0] != NULL && strcmp(args[0], "grep") == 0)
        {
                ...
                {
                        fprintf(stderr, RED "tux: grep requires two params, " ITALICS "PATTERN" RESET RED " and " RED ITALICS "FILE" RESET "\n");
                }
        }
        if (flag == 0)
...
int tux_cd(char **args)
{
        if (args[1] == NULL)
        {
                fprintf(stderr, "%stux: Please enter a path to cd%s\n", YELLOW, RESET);
        }
        else
        {
                if (chdir(args[1]) > 0)
...
char **split_line(char *line)
{
        ...
        if (!tokens)
        {
                fprintf(stderr, "%stux: Allocation error%s\n", RED, RESET);
                exit(EXIT_FAILURE);
        }
...
void loop()
{
        ...
        do
        {
                ...
                if (flag)
                {
                        ...
                        status = tux_pipe(args);
                }
                else
                {
                        args = split_line(line);
                        status = tux_launch(args);
                }
                free(line);
                free(args);
        } while (status);
